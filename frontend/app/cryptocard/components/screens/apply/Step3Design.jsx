...
export default function Step3Design({ t }) {
  ...
  const [idx, setIdx] = useState(() =>
    Math.max(
      0,
      CARD_THEMES.findIndex((th) => th.id === cardTheme)
    )
  );
  ...
  return (
    <div className={`${s["step-panel"]} ${s.active}`}>
      ...
      <div className={s["ct-toggle"]}>
        <button
          className={`${s["ct-tab"]} ${cardType === "virtual" ? s.active : ""}`}
      ...
      <div className={s["cd-info"]}>
        <div className={s["cd-name"]}>
          {cardType === "physical" ? "Matte Black" : theme.name}
        </div>
        <div className={s["cd-sub"]}>
          {cardType === "physical" ? "Standard · Physical Delivery" : theme.sub}
        </div>
      </div>
          ...
          {CARD_THEMES.map((th, i) => (
            <button
              ...
              style={
                th.bgImage
                  ? {
                      backgroundImage: `url(${th.bgImage})`,
      ...
      <div className={s["step-nav"]}>
        <button className={s["btn-back"]} onClick={prevStep} disabled={loading}>
          {t.btnBack}
        </button>
